...
/**
 * Created by dhoang9775 on 1/12/2018.
 */
...
public class PurifyParticle extends PixelParticle.Shrinking {

    public static final Emitter.Factory MISSILE = new Factory() {
        @Override
        public void emit( Emitter emitter, int index, float x, float y ) {
            ((PurifyParticle)emitter.recycle( PurifyParticle.class )).reset( x, y );
        }
    };
    ...
    public static final Emitter.Factory CURSE = new Factory() {
        @Override
        public void emit( Emitter emitter, int index, float x, float y ) {
            ((PurifyParticle)emitter.recycle( PurifyParticle.class )).resetCurse( x, y );
        }
    };
    ...
    public static final Emitter.Factory UP = new Factory() {
        @Override
        public void emit( Emitter emitter, int index, float x, float y ) {
            ((PurifyParticle)emitter.recycle( PurifyParticle.class )).resetUp( x, y );
        }
    };
    ...
    public void update() {
        ...
        color( ColorMath.interpolate( 0xF4ECBA, 0xF8F3D3, p ) );  /*0x000000, 0x440044, p used to be shadow particle colors*/
